...
import {swapTokens,updateTradeInfo,checkTokenAllowance,checkAmount,setIndependentField} from '../../store/actions/swapActions'
...
const Swap = (props) => {
	...
	const reverseTokens = () => {
		...
		props.updateTradeInfo(formattedAmounts[dependentField], isExactIn)
		props.setIndependentField(dependentField)
		setExactIn(!isExactIn)
		//props.checkAmount(formattedAmounts[independentField],independentField)
	
		console.log(trade,'---trade',isExactIn)
...
export default connect(mapStateToProps, {checkAmount,setIndependentField,checkTokenAllowance,setAmount,updateTradeInfo,swapTokens,setToAmount,setSelectedToken,setFromToken,setToToken,setFromAmount}) (Swap);
