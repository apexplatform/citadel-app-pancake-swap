...
function listToTokenMap(list: TokenList): TokenAddressMap {
  ...
  const map = list.tokens.reduce<any>((tokenMap, tokenInfo) => {
...
export function useAllLists(): any {
  return store.getState().listReducer.byUrl
}
...
export function combineMaps(map1: TokenAddressMap, map2: TokenAddressMap): TokenAddressMap {
  ...
  return chainIds.reduce<any>((memo, chainId) => {
    memo[chainId] = {
      ...map2[chainId],
...
export function useActiveListUrls(): string[] | undefined {
  return store.getState().listReducer.activeListUrls?.filter((url: any) => !UNSUPPORTED_LIST_URLS.includes(url))
}
